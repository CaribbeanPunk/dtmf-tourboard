...
st.sidebar.caption("Tip: don’t refresh constantly — cache locally.")

if refresh:
    with st.spinner("Scraping Touring Data…"):
        snap, events = scrape_all()
        if len(events) == 0:
            st.error("Scrape returned 0 events. Touring Data page structure may have changed.")
        else:
            insert_snapshot(conn, snap.__dict__)
            df_events = pd.DataFrame(events)
            upsert_events(conn, df_events)
            st.sidebar.success("Updated!")

events = read_latest_events(conn)
snaps = read_snapshots(conn)

if events.empty:
    st.info("No local data yet. Click **Refresh** in the sidebar to load the first snapshot.")
    st.stop()

roll = country_rollup(events)
...
if latest_report_data:
    ...
    components.html(html_report, height=230)
else:
...
st.markdown("#### 🗓️ Latest Runs (from latest scrape)")
